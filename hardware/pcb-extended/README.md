# AWEOMA PCB Extended

Die erweiterte PCB-Variante mit RFID, Mini-LCD und Bewegungsmelder – fuer vollstaendigen Standalone-Betrieb.

Enthaelt alle Features des **PCB Simple**, plus die unten aufgefuehrten Erweiterungen.

---

## Alle Features im Ueberblick

### Von PCB Simple uebernommen

| Feature | Detail |
|---|---|
| Stromversorgung | 5 V / 3 A via USB-C oder Barrel-Jack |
| Spannungsregler | LM2596 Schaltregler |
| 4-Pin LED | Power, VPN, WAN, Error |
| 2x PWM-Ausgang | GPIO 12 und GPIO 13 |
| Touch-Sensor | TTP223 kapazitiver Touch – GPIO 24 |
| 5V-Ausgang | Geregelter Ausgang fuer externe Komponenten |
| Reset-Taster | Momentary Push-Button – GPIO 3 |
| Verpolungsschutz | P-MOSFET Eingangsschutz |

### Zusaetzlich (Extended)

| Zusatz-Feature | Detail |
|---|---|
| **RFID-Reader** | RC522 Modul via SPI (MISO/MOSI/SCK/CE0) fuer Zugangskontrolle |
| **Mini-LCD** | 0.96" SSD1306 OLED oder 1.3" I2C-LCD via I2C (SDA/SCL) |
| **Bewegungsmelder** | HC-SR501 PIR-Sensor – GPIO 25 |

---

## GPIO-Belegung (Komplett)

| GPIO (BCM) | Pin | Funktion | Richtung | Beschreibung |
|---|---|---|---|---|
| GPIO 2 (SDA) | 3 | I2C SDA | I/O | Display (SSD1306 / LCD) |
| GPIO 3 (SCL) | 5 | I2C SCL + Reset | I/O | Display + Reset-Taster (shared) |
| GPIO 8 (CE0) | 24 | SPI CS | Output | RFID RC522 Chip Select |
| GPIO 9 (MISO) | 21 | SPI MISO | Input | RFID Daten empfangen |
| GPIO 10 (MOSI) | 19 | SPI MOSI | Output | RFID Daten senden |
| GPIO 11 (SCLK) | 23 | SPI SCLK | Output | RFID Takt |
| GPIO 12 | 32 | PWM0 | Output | Hardware-PWM Kanal 0 |
| GPIO 13 | 33 | PWM1 | Output | Hardware-PWM Kanal 1 |
| GPIO 17 | 11 | LED Power | Output | Gruen – System laeuft |
| GPIO 22 | 15 | LED WAN | Output | Gelb – Netzwerkverbindung |
| GPIO 23 | 16 | LED Error | Output | Rot – Fehler / Alarm |
| GPIO 24 | 18 | Touch-Sensor | Input | Kapazitiver Touch (TTP223) |
| GPIO 25 | 22 | PIR-Sensor | Input | Bewegungsmelder HC-SR501 |
| GPIO 27 | 13 | LED VPN | Output | Blau – VPN aktiv |

> Hinweis: GPIO 3 wird fuer Reset UND I2C SCL verwendet. Auf dem Extended-PCB ist der Reset-Taster ueber einen extra Pulldown-Widerstand entkoppelt, um Konflikte zu vermeiden. Bei gleichzeitiger I2C-Nutzung ist ein seperater Reset-Pin (GPIO 4) als Alternative moeglich.

---

## Stückliste (BOM) – Zusaetzbauteile gegenueber Simple

| Ref. | Bauteil | Wert / Typ | Menge |
|---|---|---|---|
| U3 | RFID-Reader Modul | RC522 (SPI, 3.3V) | 1 |
| U4 | OLED-Display | SSD1306 0.96" I2C 128x64 | 1 |
| U5 | PIR-Sensor | HC-SR501 (einstellbare Empfindlichkeit) | 1 |
| C5 | Entkopplungs-Kondi RFID | 100 nF (0805) | 1 |
| R6 | I2C Pull-up SDA | 4.7 kOhm | 1 |
| R7 | I2C Pull-up SCL | 4.7 kOhm | 1 |
| J5 | RFID-Anschlussstecker | 8-Pin Male Header 2.54 mm | 1 |
| J6 | Display-Anschlussstecker | 4-Pin Male Header 2.54 mm (VCC, GND, SDA, SCL) | 1 |
| J7 | PIR-Anschlussstecker | 3-Pin Male Header 2.54 mm (VCC, GND, OUT) | 1 |

Vollstaendige BOM: `bom/pcb-extended-bom.csv`

---

## Software-Beispiele

### RFID-Reader (RC522)

```python
import RPi.GPIO as GPIO
import spidev
# Bibliothek: pip install mfrc522
from mfrc522 import SimpleMFRC522

reader = SimpleMFRC522()
try:
    print("Karte an RFID-Reader halten...")
    id, text = reader.read()
    print(f"ID: {id}")
    print(f"Text: {text}")
finally:
    GPIO.cleanup()
```

SPI aktivieren: `sudo raspi-config` → Interfaces → SPI → Enable

### OLED-Display (SSD1306)

```python
# Bibliotheken: pip install luma.oled
from luma.core.interface.serial import i2c
from luma.oled.device import ssd1306
from luma.core.render import canvas
from PIL import ImageFont

serial = i2c(port=1, address=0x3C)
device = ssd1306(serial)

# IP-Adresse und VPN-Status anzeigen
import socket
hostname = socket.gethostname()
ip = socket.gethostbyname(hostname)

with canvas(device) as draw:
    draw.text((0, 0),  "AWEOMA Gateway", fill="white")
    draw.text((0, 16), f"IP: {ip}", fill="white")
    draw.text((0, 32), "VPN: aktiv", fill="white")
    draw.text((0, 48), "Pi-hole: ON", fill="white")
```

I2C aktivieren: `sudo raspi-config` → Interfaces → I2C → Enable

### Bewegungsmelder (HC-SR501 PIR)

```python
import RPi.GPIO as GPIO
import time

PIR_PIN = 25
GPIO.setmode(GPIO.BCM)
GPIO.setup(PIR_PIN, GPIO.IN)

print("PIR-Sensor wird initialisiert (30 Sekunden warten)...")
time.sleep(30)
print("Bereit – warte auf Bewegung...")

def bewegung_erkannt(channel):
    print("Bewegung erkannt!")
    # z.B. LED einschalten, Benachrichtigung senden, etc.
    GPIO.output(23, GPIO.HIGH)  # Error-LED als Alarm

GPIO.add_event_detect(PIR_PIN, GPIO.RISING, callback=bewegung_erkannt, bouncetime=2000)

try:
    while True:
        time.sleep(1)
finally:
    GPIO.cleanup()
```

---

## Systemdienst: Display-Status

Autostart-Service fuer die OLED-Statusanzeige:

```ini
# /etc/systemd/system/aweoma-display.service
[Unit]
Description=AWEOMA OLED Status Display
After=network.target wg-quick@wg0.service

[Service]
Type=simple
User=pi
ExecStart=/usr/bin/python3 /opt/aweoma/scripts/display.py
Restart=always
RestartSec=5

[Install]
WantedBy=multi-user.target
```

```bash
sudo systemctl enable aweoma-display
sudo systemctl start aweoma-display
```

---

## Fertigungsparameter

| Parameter | Wert |
|---|---|
| Lagen | 2 (FR4) |
| Dicke | 1.6 mm |
| Kupferdicke | 1 oz |
| Loetstopplack | Schwarz (fuer Premium-Look) oder Gruen |
| Silkscreen | Weiss |
| Mindest-Leiterbahn | 0.15 mm (wegen SPI-Leitungen) |
| Mindest-Bohrung | 0.3 mm |

Gerber-Dateien: `gerber/pcb-extended-gerber.zip`

---

> Fotos und 3D-Renders folgen nach der ersten Fertigung.
